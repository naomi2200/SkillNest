...
@section('dashboard-title', 'Pagar mentoría')

@section('dashboard-content')
    <div class="mx-auto max-w-3xl space-y-6">
        <a href="{{ route('student.mentorias') }}" class="text-sm font-semibold text-secondary hover:underline">← Volver a mis mentorías</a>

        <div class="card space-y-6">
            <div>
                <p class="text-xs uppercase tracking-[0.3em] text-slate-400">Mentor</p>
                <h1 class="text-3xl font-semibold text-secondary">{{ $mentoria->mentor->name ?? 'Mentor SkillNest' }}</h1>
                <p class="text-sm text-slate-500">{{ $mentoria->especialidad ?? 'Generalista' }}</p>
            </div>

            <div class="rounded-2xl border border-slate-100 bg-slate-50/70 p-4 text-sm text-slate-600">
                <div class="flex justify-between">
                    <span>Monto total</span>
                    <strong class="text-secondary">S/ {{ number_format($monto, 2) }}</strong>
                </div>
            ...
            <form method="POST"
                ...
                <div>
                    ...
                    <select name="metodo" id="metodo_pago" class="form-input" required>
                        ...
                        <option value="yape" @selected($selectedMethod === 'yape')>Yape</option>
                        <option value="plin" @selected($selectedMethod === 'plin')>Plin</option>
                        <option value="tarjeta" @selected($selectedMethod === 'tarjeta')>Tarjeta</option>
                    </select>
                </div>

                <div id="capture_wrapper" style="display: {{ $showCapture ? 'block' : 'none' }};">
                ...
                <div id="tarjeta_wrapper" style="display: {{ $showCard ? 'block' : 'none' }};">
