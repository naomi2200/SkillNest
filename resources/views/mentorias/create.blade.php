...
@section('mentor-title', 'Crear mentor&iacute;a')
@section('mentor-subtitle', 'Define cada detalle de tu nueva sesi&oacute;n personalizada')

@section('mentor-actions')
    <a href="{{ route('mentor.mentorias.index') }}" class="btn btn-secondary" style="border-radius: 999px; padding: 10px 22px;">
        ...
        <div class="mentoria-create-shell">
            ...
            <div class="mentoria-create-grid">
                <div class="mentoria-card">
                    <form action="{{ route('mentorias.store') }}" method="POST">
                        @csrf
                        <div class="form-section">
                            <label for="titulo" class="form-label">T&iacute;tulo</label>
                            <input id="titulo" name="titulo" type="text" class="form-field" value="{{ old('titulo') }}" required>
                        </div>
                        <div class="form-section">
                            <label for="especialidad" class="form-label">Especialidad / tem&aacute;tica</label>
                            <input id="especialidad" name="especialidad" type="text" class="form-field" value="{{ old('especialidad') }}" placeholder="Ej: Finanzas personales, UX Research" required>
                        </div>
                        <div class="form-grid form-section">
                            ...
                            <div>
                                ...
                                <input id="precio" name="precio" type="number" step="0.01" class="form-field" value="{{ old('precio') }}" required>
                            </div>
                            <div>
                                <label for="duracion_minutos" class="form-label">Duraci&oacute;n (min)</label>
                                <input id="duracion_minutos" name="duracion_minutos" type="number" min="15" class="form-field" value="{{ old('duracion_minutos', 60) }}" required>
                            </div>
                        </div>
                        <div class="form-section">
                            <label for="descripcion" class="form-label">Descripci&oacute;n</label>
                            <textarea id="descripcion" name="descripcion" rows="5" class="form-field" required>{{ old('descripcion') }}</textarea>
                        </div>
                        <div class="mentoria-actions">
                            <a href="{{ route('mentor.mentorias.index') }}" class="btn-glass">Cancelar</a>
