...
@section('student-content')
    <div class="profile-page">
        {{-- Page header removed to avoid duplicate title; layout now shows the title/subtitle. --}}
        ...
        <div class="profile-header-card">
            <div style="display:flex; gap:1rem; align-items:center">
                ...
                <div style="display:flex; flex-direction:column">
                    <h2 class="profile-name">{{ $user->name ?? 'Estudiante' }}</h2>
                    <p class="profile-subtitle">{{ $user->profile_title ?? 'Estudiante en SkillNest' }}</p>
                    <div class="profile-stats" style="margin-top:.5rem">
                        <div class="stat-item">
                            <div class="stat-label">Miembro desde</div>
                            <div class="stat-value">{{ optional($user->created_at)->format('d M Y') ?? '—' }}</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">Cursos</div>
                            <div class="stat-value">{{ $user->cursosInscritos?->count() ?? 0 }}</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">Mentorías</div>
                            <div class="stat-value">{{ $user->mentoriasComoEstudiante?->count() ?? 0 }}</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="country" class="form-select">
                            ...
                            @foreach(['colombia'=>'Colombia','peru'=>'Perú','mexico'=>'México','argentina'=>'Argentina','chile'=>'Chile'] as $k=>$v)
                                <option value="{{ $k }}" @selected(old('country', $user->country) == $k)>{{ $v }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="specialty" class="form-select">
                            ...
                            <option value="fullstack" @selected(old('specialty', $user->specialty)=='fullstack')>Full Stack Development</option>
                            <option value="frontend" @selected(old('specialty', $user->specialty)=='frontend')>Frontend Development</option>
                            <option value="backend" @selected(old('specialty', $user->specialty)=='backend')>Backend Development</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea name="bio" class="form-textarea">{{ old('bio', $user->bio) }}</textarea>
                    </div>
                </div>
