            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($courses as $course)
                    <tr>
                        ...
                        @endphp
                        <td class="py-4">
                            <span class="badge {{ $statusColors[$course->status] ?? 'bg-slate-100 text-slate-500' }}">
                                {{ ucfirst($course->status) }}
                            </span>
                        </td>
                        <td class="py-4 text-right space-x-2">
                            <a href="{{ route('cursos.editor', $course) }}" class="text-sm font-semibold text-primary hover:text-primary/80">Editar</a>
