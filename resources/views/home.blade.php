    ...
    <div class="home-wrapper">
        <section class="home-hero-grid">
            <div class="card home-hero-card">
                ...
                <form action="{{ route('cursos.index') }}" method="GET" class="home-hero-form">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('cursos.index') }}" class="btn-primary">Ver cursos publicados</a>
                    <a href="{{ route('mentor-market.index') }}" class="btn-secondary">Ver mentorías</a>
                </div>
        ...
        <section class="space-y-6">
            <div class="home-section-header">
                ...
                <a href="{{ route('cursos.index') }}" class="btn-secondary">Ver todos</a>
            </div>
            <div class="home-category-grid">
                @foreach($categories as $category)
                    <article class="card home-category-card">
                        ...
                        <p class="home-section-text text-sm">{{ $category['courses'] }} cursos</p>
                    </article>
                @endforeach
            </div>
        </section>
                ...
                @foreach($courses as $course)
                    <article class="card home-course-card">
                        ...
                        <div class="home-course-content p-6">
                            <span class="btn-secondary px-3 py-1 inline-flex justify-center">{{ $course['category'] }}</span>
                            <div>
                                <h3 class="text-xl font-semibold text-secondary">{{ $course['title'] }}</h3>
                                <p class="home-section-text text-sm">{{ $course['mentor'] }}</p>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('cursos.index') }}" class="btn-primary">Ver más</a>
                            </div>
                        </div>
                    </article>
                @endforeach
        ...
        <section class="space-y-6">
            <div class="home-section-header">
                ...
                <a href="{{ route('mentor-market.index') }}" class="btn-secondary">Ver todos</a>
            </div>
            <div class="home-mentor-grid">
                @foreach($mentors as $mentor)
                    <article class="card home-mentor-card">
                        <div class="home-mentor-avatar">{{ substr($mentor['name'], 0, 1) }}</div>
                        <h3 class="text-lg font-semibold text-secondary">{{ $mentor['name'] }}</h3>
                        <p class="home-section-text text-sm">{{ $mentor['title'] }}</p>
                        <div class="flex items-center justify-center gap-4 text-sm text-slate-500">
                            <span>⭐ {{ $mentor['rating'] }}</span>
                            <span>👥 {{ $mentor['sessions'] }}</span>
                        </div>
                        <p class="text-2xl font-bold text-secondary">S/ {{ $mentor['price'] }}/hora</p>
                        <a href="{{ route('mentor-market.index') }}" class="btn-primary">Ver perfil</a>
                    </article>
                @endforeach
            </div>
        </section>
        ...
        <section class="home-cta">
            <div class="space-y-4">
                ...
                <a href="{{ route('cursos.index') }}" class="btn-secondary border-white text-white">Explorar cursos</a>
            </div>
            <div class="space-y-4">
                ...
                <a href="{{ route('register') }}" class="btn-secondary border-white text-white">Comenzar ahora</a>
            </div>
        </section>
    </div>
@endsection
