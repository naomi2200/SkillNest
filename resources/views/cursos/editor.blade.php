    ...
    <div class="flex w-full flex-wrap items-center gap-4 rounded-[32px] border border-slate-100 bg-white/80 px-5 py-4 shadow-card">
        <div class="flex flex-1 flex-wrap items-center gap-3">
            <div class="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">
                Estado
                <span class="rounded-full bg-secondary/10 px-3 py-0.5 text-secondary capitalize" id="course-status">{{ $curso->status }}</span>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('cursos.show', $curso) }}" class="btn-secondary rounded-full border border-slate-200 px-5 py-2 text-sm font-semibold">Vista previa</a>
    ...
    <div class="mb-6 grid gap-4 lg:grid-cols-3">
        ...
        <div class="rounded-[28px] border border-slate-100 bg-white/90 px-6 py-5 shadow-card">
            ...
            <div class="mt-2 flex items-center gap-3">
                <p class="text-3xl font-bold text-secondary">{{ $modulesCount }}</p>
        ...
        <div class="rounded-[28px] border border-slate-100 bg-white/90 px-6 py-5 shadow-card">
            ...
            <div class="mt-2 flex items-center gap-3">
                <p class="text-3xl font-bold text-secondary">{{ $lessonsCount }}</p>
                ...
                @foreach($basicFields as $field)
                    <label class="block text-xs font-semibold text-secondary">
                        ...
                        @if($field['type'] === 'select')
                            <select data-basic-field="{{ $field['name'] }}" class="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm">
                                @foreach($field['options'] as $option)
                                    <option value="{{ $option }}" @selected($curso->{$field['name']} === $option)>{{ ucfirst($option) }}</option>
                                @endforeach
                            </select>
                        @elseif($field['type'] === 'textarea')
                            <textarea data-basic-field="{{ $field['name'] }}" class="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm" rows="3">{{ $curso->{$field['name']} }}</textarea>
                        @else
                    ...
                    <div class="module-card rounded-[32px] border border-slate-200 bg-white px-5 py-6 shadow-card" data-module-id="{{ $module->id }}">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1 space-y-3">
                                ...
                                <textarea class="module-description w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm" rows="2" placeholder="Descripcion del modulo (opcional)">{{ $module->description }}</textarea>
                            </div>
                        ...
                        <div class="lessons-wrapper mt-4 space-y-3" data-lessons-container>
                            @foreach($module->lessons as $lesson)
                                <div class="lesson-block rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4" data-lesson-id="{{ $lesson->id }}">
                                    <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="space-y-3">
                ...
                <div class="space-y-2 text-xs text-slate-500">
                    <div class="flex items-center justify-between">
                        <span>Borrador</span>
                        <span>{{ $modulesCount > 0 ? 'Completado' : 'Pendiente' }}</span>
                    </div>
                    <div class="h-2 rounded-full bg-slate-100">
                        <div class="h-2 rounded-full bg-emerald-400" style="width: {{ $modulesCount ? '70%' : '30%' }}"></div>
                    </div>
                    <div class="flex items-center justify-between">
                        <span>Revision</span>
                    ...
                    <div class="h-2 rounded-full bg-slate-100">
                        <div class="h-2 rounded-full bg-amber-400" style="width: {{ $curso->status === 'pendiente' ? '80%' : '35%' }}"></div>
                    </div>
                </div>
            </div>
        </aside>
