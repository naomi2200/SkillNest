        ...
        <div class="course-main-grid">
            <div>
                <div class="course-image-container">
                    @if ($cover)
                        <img src="{{ $cover }}" alt="Imagen del curso {{ $curso->title }}">
                    @else
                ...
                <article class="course-section-card">
                    ...
                    <div class="learning-grid">
                        <div class="learning-item">
                            ...
                            <span>Construir proyectos completos con {{ $curso->title }}</span>
                        </div>
                        <div class="learning-item">
                            <div class="learning-icon">✓</div>
                    ...
                    <div class="instructor-card">
                        ...
                        <div>
                            <p style="font-size: 1.2rem; font-weight: 700; color: #1f2937; margin: 0 0 6px;">
                                {{ $curso->mentor->name ?? 'Mentor por asignar' }}
                            </p>
                            <p style="font-size: 0.9rem; color: #6b7280; margin: 0;">
                                Especialista en {{ $curso->category ?? 'la temática del curso' }}
                            </p>
                        </div>
                    </div>
                </article>
            ...
            <aside class="course-sidebar">
                <div class="price-card">
                    <div class="price-header">
                        ...
                        <div class="price-amount">S/ {{ number_format($curso->price, 2) }}</div>
                    </div>

                    <div class="price-actions">
                        @auth
                            @if($curso->isPurchasedBy(auth()->user()))
                                <a href="{{ route('courses.classroom', $curso) }}" class="btn-primary">Ir al aula</a>
                            @else
                                <a href="{{ route('courses.checkout', $curso) }}" class="btn-primary">Comprar curso</a>
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="btn-primary">Ingresa para inscribirte</a>
                        @endauth
                        @can('update', $curso)
                            <a href="{{ route('cursos.editor', $curso) }}" class="btn-secondary">Editar curso</a>
                        @endcan
                    </div>
                ...
                <div class="details-card">
                    ...
                    <div class="details-list">
                        <div class="detail-item">
                            ...
                            <span class="detail-value">{{ $curso->duration }} horas</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Mentor</span>
                            <span class="detail-value">{{ $curso->mentor->name ?? 'No asignado' }}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Nivel</span>
                            <span class="detail-value">{{ ucfirst($curso->level ?? 'Todos') }}</span>
                        </div>
                        <div class="detail-item">
                            <span class="detail-label">Estado</span>
                            <span class="detail-value">{{ ucfirst($curso->status) }}</span>
                        </div>
                    </div>
                </div>
            </aside>
